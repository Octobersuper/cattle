layui.use(['form','layer','layedit','laydate','upload'],function(){
    ...
    form.on("submit(addNews)",function(data){
        ...
        if(files.length > 0) {
            ...
            formData.append('title', data.field.title);//标题
            formData.append('rent', data.field.rent);
            formData.append('deposit', data.field.deposit);
            formData.append('contact', data.field.contact);
            formData.append('remark', data.field.remark);//描述
            $.ajax({
                type: "post",
                url: baseUrl+"/lease/insert",
                data: formData,
                async: true,
                processData: false,
                contentType: false,
