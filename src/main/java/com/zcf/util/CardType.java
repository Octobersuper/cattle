...
public class CardType {
	...
	public static int getOdds(int[] brand, RoomBean myRoom) {
		 ...
		 */
		String[] strings = myRoom.getBrand_type();
		if (strings != null) {
			for (int i = 0; i < strings.length; i++) {
				if (Integer.parseInt(strings[i]) == 1 || Integer.parseInt(strings[i]) == 4) {
				...
				if (Integer.parseInt(strings[i]) == 2) {
					if (400 > odds && odds > 300)
						return 5;

				}
				if (Integer.parseInt(strings[i]) == 3) {
					if (700 > odds && odds > 600)
						return 7;

				}
				if (Integer.parseInt(strings[i]) == 5) {
					if (900 > odds && odds > 800)
						return 8;

				}
				if (Integer.parseInt(strings[i]) == 6) {
